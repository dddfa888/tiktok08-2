...
export const _bindPhone = (params) => {
  return request({
    url: '/wap/api/localuser!bindPhone.action',
    method: METHODS.POST,
    data: params
  })
}
...
export const _applyIdentify = (data) => {
  const params = {
    ...
    name: data.name, // 姓名
    yqm: data.yqm,
    idimg_1: data.frontFile.length
      ? data.frontFile[0].resURL || data.frontFile[0].url
      : '',
    idimg_2: data.reverseFile.length
      ? data.reverseFile[0].resURL || data.reverseFile[0].url
      : ''
...
export const customerServiceUrl_st = () => {
  return request({
    url: '/wap/api/syspara!getSyspara.action?code=customer_service_url_st',
    loadingPass: true,
    method: 'GET'
  })
}
