<template>
  <div class="h-full w-full">
    ...
    <van-cell-group>
      <van-cell style="min-height: 3.125rem" :title="$t('realName')" is-link :to="'/name'">
        <div class="verify-info-content" :class="{ 'is-ar': isArLang }">
          ...
          <div v-if="kycStatusTxt" class="icon-content">
            <img
              :src="kycStatus === 2 ? defaultAvatar.verifyIcon2 : (kycStatus === 1 ? defaultAvatar.verifyIcon3 : defaultAvatar.verifyIcon1)"
              alt="">
            <p>{{ t(kycStatusTxt) }}</p>
          </div>
        </div>
      </van-cell>
      <van-cell v-if="!isFM" style="min-height: 3.125rem"
        :title="notAllowPhoneClick ? $t('phoneNum') : $t('phoneVerify')" :is-link="!notAllowPhoneClick"
        :to="verifyRoute(1, notAllowPhoneClick)">
        <div class="verify-info-content" :class="{ 'is-ar': isArLang }">
      ...
      <van-cell v-if="!isFM" style="min-height: 3.125rem" :title="notAllowEmailClick ? $t('邮箱') : $t('emailVerify')"
        :is-link="!notAllowEmailClick">
        <!-- :to="verifyRoute(2, notAllowEmailClick) -->
        <div class="verify-info-content" :class="{ 'is-ar': isArLang }">
...
const verifyRoute = (type, flag) => {
  if (!flag) {
    ...
    if (!userInfo.value.phone) {
      return goCertified ? `certified?type=${type}` : `bindVerify?type=${type}&verType=${typeNum}`
    }
  }
}
