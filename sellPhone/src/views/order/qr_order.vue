    ...
    <div class="line-content" :class="{'is-ar': isArLang}" v-if="Object.keys(orderInfo).length > 0">
      <div class="line">
        ...
        <div v-if="orderInfo.sellerDiscountPrice" class="price-content">
          <!-- numberStrFormat(orderInfo.sellerDiscountPrice) -->
          {{ t('优惠价') }}<span>${{ orderInfo.sellerDiscountPrice }}</span>, {{ t('采购优惠') }}<span>{{ new Decimal(orderInfo.sellerDiscount).mul(100) }}%</span>
        </div>
        <div class="line-item">
          <p>{{ $t('采购数量') }}</p>
        ...
        <div class="line-item">
          <p>{{ $t('运费') }}</p>
          <p class="count">${{ numberStrFormat(orderInfo.sellerFees) }}</p>
        </div>
        <div class="line-item">
          <p>{{ $t('税') }}</p>
