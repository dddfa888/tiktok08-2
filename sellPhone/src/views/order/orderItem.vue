  ...
  <div class="goods_list" :class="{ 'is-ar': isArLang }">
    ...
    <div @click="goDetails">
      ...
      <p class="mb-2">
        ...
        <span>{{ $t('支付状态') }}：
          <span :style="{ 'color': props.info.payStatus == 1 ? '#2369f6' : '#dc2626' }">{{ props.info.payStatus == 1 ?
            $t('买家已付款') : $t('等待买家付款') }}</span>
      ...
      <div class="bottom-content">
        ...
        <div class="btn-content">
          ...
          <span v-if="Number(props.info.purchStatus) !== 1 && ![-1, 0, 6].includes(Number(props.info.status))"
            class="btn" @click.stop="buy">{{ $t('采购') }}</span>
        </div>
      </div>
    </div>
  </div>
