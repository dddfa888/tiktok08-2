<template>
  <div>
    ...
    <div class="section" :class="{'is-ar': isArLang}">
      ...
      <div class="info-list">
        <p>{{ $t('姓名') }}</p>
        <div>{{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.username }}</div>
      </div>
      <div class="info-list">
        <p>{{ $t('邮箱') }}</p>
      ...
      <div class="info-list">
        <p>{{ $t('国家') }}</p>
        <div>{{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.country }}</div>
      </div>
      <div class="info-list">
        <p>{{ $t('州') }}</p>
        <div>{{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.province }}</div>
      </div>
      <div class="info-list">
        <p>{{ $t('城市') }}</p>
        <div>{{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.city }}</div>
      </div>
      <div class="info-list">
        <p>{{ $t('邮编') }}</p>
    ...
    <div class="section bottom" :class="{'is-ar': isArLang}">
      ...
      <div class="list">
        {{ $t('运费') }} <span>${{ numberStrFormat(orderInfo.sellerFees) }}</span>
      </div>
      <!--      <div class="list">-->
...
const totalNum = computed(() => {
  const { sellerDiscountPrice, systemPrice, sellerFees, tax } = orderInfo.value
  const price = sellerDiscountPrice ? sellerDiscountPrice : systemPrice
  return numberStrFormat(Number(price) + Number(sellerFees) + Number(tax))
})
