<template>
  <div>
    <fx-header :fixed="true">
      <template #title>{{ t('realNameVertify') }}</template>
    </fx-header>
    ...
    <div
      v-if="!pageLoading"
      class="identity pl-15 pr-15 font-12 bg-white"
      style="padding-bottom: 30px;"
    >
      <div
        v-if="kycInfo.showStatus"
        class="flex justify-between items-center text-sm pt-6"
        style="color: #000"
      >
        <div>{{t('authVerify')}}</div>
        <div class="flex items-center">
          <img
            v-if="kycInfo.status === 1"
            class="w-4 h-4"
            :class="isArLang ? 'ml-0.5': 'mr-0.5'"
            :src="KeyWait"
            alt
          />
          <img
            v-if="kycInfo.status === 2"
            class="w-4 h-4"
            :class="isArLang ? 'ml-0.5': 'mr-0.5'"
            :src="KeySuccess"
            alt
      ...
      <div v-if="kycInfo.status === 3 && kycInfo.msg" style="margin-top: 10px;">
        <van-notice-bar wrapable :scrollable="false" :text="kycInfo.msg" />
      </div>

      <div style="padding-top: 20px;">
        <div class="mb-5">
          <div
            class="mt-27 mb-13 font-12 textColor form-item-title"
        ...
        <ExInput
          :label="t('realName')"
          :placeholderText="t('entryRealName')"
          :required="true"
          v-model="name"
          :disabled="kycInfo.disabled"
          :clearBtn="!kycInfo.disabled"
        />
        <ExInput
          :label="t('credentPassport')"
          :placeholderText="t('entryCredentPassport')"
          :maxLength="40"
          :required="true"
          v-model="idnumber"
          :disabled="kycInfo.disabled"
            ...
            <div class="flex-1 flex flex-col text-center justify-center items-center">
              <div class="upload-wrap">
                <van-uploader
                  ...
                  :max-count="1"
                  :disabled="kycInfo.disabled"
                  :deletable="!kycInfo.disabled"
                  :after-read="(file) => afterRead(file, frontFile, 'idimg_1')"
                />
              </div>
            ...
            <div class="flex-1 flex flex-col text-center justify-center items-center">
              <div class="upload-wrap">
                <van-uploader
                  ...
                  :disabled="kycInfo.disabled"
                  :deletable="!kycInfo.disabled"
                  :after-read="(file) => afterRead(file, reverseFile, 'idimg_2')"
                />
              </div>
  ...
  _getIdentify()
    .then((data) => {
      ...
      if (data.status !== 0) {
        ...
        idnumber.value =
          data.idnumber && [1, 2].includes(status)
            ? formeateUser(data.idnumber, false)
            : data.idnumber || ''
        name.value =
          data.name && [1, 2].includes(status)
            ? formeateUser(data.name, false)
            : data.name || ''
...
const afterRead = (file, type, moduleName) => {
  /// 处理文件
