    ...
    <div class="content">
      ...
      <div class="level-number-content">
        ...
        <div v-if="currentLevelObj" @click="openPage('/sellerLevel')">
          <p>
            {{ t('当前分店人数') }}
            <span>{{ currentChildNum }}</span>
            <span
              v-if="currentLevelObj.popularizeUserCountCnd"
              class="em"
            >/{{ currentLevelObj.popularizeUserCountCnd }}</span>
            ,
            {{ t('当前团队人数') }}
            <span>{{ currentTeamNum }}</span>
            <span v-if="currentLevelObj.teamNum" class="em">/{{ currentLevelObj.teamNum }}</span>
            ,
            {{ t('当前采购优惠') }}
            ...
            <div class="flex justify-between items-center" style="flex-wrap: wrap">
              <div>
                <div class="flex items-center freeze-total-content">
                  ...
                  >{{ showWalletNum ? `$${numberStrFormat(userInfo.money)}` : '*****' }}</div>
                  <i
                    v-if="showWalletNum"
                ...
                <div class="freeze-num-content">
                  {{ t('balance') }}
                  <p v-if="Number(userInfo.frozenMoney)">
                    (
                    <span>{{ t('待到账金额') }} {{ showWalletNum ? `$${numberStrFormat(userInfo.frozenMoney)}` : '*****' }}</span>)
                  </p>
                  <p v-if="Number(userInfo.purchase)">
                    (
                    <span>{{ t('采购资金') }} {{ showWalletNum ? `$${numberStrFormat(userInfo.purchase)}` : '*****' }}</span>)
                  </p>
                </div>
              </div>
            </div>
      ...
      <div v-if="showRward || showInvite" class="award-content">
        <van-swipe class="my-swipe" :autoplay="5000" indicator-color="white">
          ...
          <van-swipe-item v-if="showInvite">
            <div class="invite-content" @click.stop="openPage('/invitation-activity')">
              <h3 v-html="t('myInviteTitle')" :class="{ 'not-cn': notCnLang }"></h3>
              <p
                v-html="
                  t('myInviteInfo', {
              ...
              >{{ inviteAmountReward ? t('领取') : t('邀请') }}</div>
            </div>
          </van-swipe-item>
        </van-swipe>
      </div>
          ...
          <van-cell is-link to="/fundsRecords">
            <template #title>
              <div class="flex items-center">
                ...
                <div class="color-333" :class="isArLang ? 'mr-2' : 'ml-2'">{{ t('fundingRecords') }}</div>
              </div>
            </template>
          </van-cell>
