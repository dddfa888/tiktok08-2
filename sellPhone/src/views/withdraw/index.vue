  ...
  <div>
    ...
    <div class="withdraw" :class="{ 'is-ar': isArLang }">
      <div class="icon-type-content">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('取款方式') }}</div>
        <van-field v-model="withdrawType" readonly name="picker" @click="typePicker = true" />
      ...
      <div v-if="withdrawTypeVal === 'icon'" class="icon-type-content">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('选择币种') }}</div>
        <van-field v-model="typeValue" readonly name="picker" @click="openShowPicker" />
      ...
      <van-form v-if="withdrawTypeVal === 'icon'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('withdrawalAddress') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field
            clearable
            v-model="from"
            :disabled="isBindType"
      ...
      <van-form v-if="withdrawTypeVal === 'bank' && showBankNational">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('国家') }}</div>
        <div class="mb-5 nationality-select">
          <country-select v-model="nationality" get-name="true" @done="countrySelectDone" />
        </div>
      </van-form>
      <van-form v-if="withdrawTypeVal === 'bank'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('开户行') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="bankName" :placeholder="t('请输入开户行名称')" />
        </div>
      </van-form>
      <!-- 当取款方式选择的是银行卡或者支付宝 -->
      <van-form v-if="withdrawTypeVal === 'bank' || withdrawTypeVal === 'zfb'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('姓名') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="bankUserName" :placeholder="t('entryName')" />
        </div>
      </van-form>
      <van-form v-if="withdrawTypeVal === 'bank'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('卡号') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="bankCardNo" :placeholder="t('请输入卡号')" />
        </div>
      </van-form>

      <van-form v-if="withdrawTypeVal === 'zfb'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('支付宝账号') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="bankCardNo" :placeholder="t('请输入账号')" />
        </div>
      </van-form>
      <van-form v-if="withdrawTypeVal === 'wx'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ t('微信绑定手机号') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="bankCardNo" :placeholder="t('请输入账号')" />
        </div>
      </van-form>
      ...
      <van-form v-if="withdrawTypeVal === 'wx'">
        <div class="mb-2.5 text-xs required-txt" style="color: #333">{{ $t('上传微信收款码（需保存收款码到相册)') }}</div>
        <van-uploader
          v-model="fileList"
      ...
      <van-form v-if="withdrawTypeVal === 'bank' && showBankMoreInfo">
        <div class="mb-2.5 text-xs" style="color: #333">{{ t('国际代码') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="swiftCode" :placeholder="t('请输入国际代码')" />
        </div>
      </van-form>
      <van-form v-if="withdrawTypeVal === 'bank' && showBankMoreInfo">
        <div class="mb-2.5 text-xs" style="color: #333">{{ t('路由号码') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="routingNum" :placeholder="t('请输入路由号码')" />
        </div>
      </van-form>
      <van-form v-if="withdrawTypeVal === 'bank' && showBankMoreInfo">
        <div class="mb-2.5 text-xs" style="color: #333">{{ t('账户地址') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="accountAddress" :placeholder="t('请输入账户地址')" />
        </div>
      </van-form>
      <van-form v-if="withdrawTypeVal === 'bank' && showBankMoreInfo">
        <div class="mb-2.5 text-xs" style="color: #333">{{ t('银行地址') }}</div>
        <div class="mb-5 border border-solid rounded" style="border-color: #eeeeee">
          <van-field clearable v-model="bankAddress" :placeholder="t('请输入银行地址')" />
        </div>
      </van-form>
      ...
      >
        {{ t('当前余额') }}:
        <span>{{ numberStrFormat(balanceAll) }}</span>USDT ≈
        <span>{{ numberStrFormat(balanceTypeAll, balanceFixed) }}</span>
        {{ typeValue }}
      </div>
      <div v-if="withdrawTypeVal !== 'icon'" class="current-balance">
        {{ t('当前余额') }}:
        <span>{{ numberStrFormat(balanceAll) }}</span>USD
      </div>
      ...
      <div class="tips">
        <div>
          ...
          <p
            v-if="(withdrawTypeVal =='icon' && typeValue !== 'USDT') || withdrawTypeVal !== 'bank'"
            class="money"
          >
            ≈
            <span>{{ realWithdrawAmountUsdt }}</span>USDT
          </p>
        </div>
      ...
      <van-button
        ...
        @click="withdraw"
      >{{ t('submit') }}</van-button>
      <van-button type="primary" :disabled="notSub" @click="openService(false)">{{ t('联系客服') }}</van-button>
...
const withdrawAmountMax3 = ref('0.00')
...
const getWithdrawLimit = () => {
  exchangeGetWithdrawLimit().then((res) => {
    ...
    withdrawAmountMax3.value = res.withdrawAmountMax3 || 0
...
watch(amount, (newVal) => {
  if (withdrawTypeVal.value == 'zfb') {
    // console.log(Number(newVal),'输入金额')
    if (Number(newVal) < withdrawAmountMin2.value) {
      Toast(
        t(`提款金额不得低于最小提款限额`) + `: ${withdrawAmountMin2.value} USDT`
      )
    } else if (Number(newVal) > withdrawAmountMax3.value) {
      Toast(
        t(`提款金额不得高于最大提款限额`) + `: ${withdrawAmountMax3.value} USDT`
      )
    }
  } else {
    if (Number(newVal) < withdrawAmountMin.value) {
      console.log(withdrawAmountMin2.value)
      Toast(
        t(`提款金额不得低于最小提款限额`) + `: ${withdrawAmountMin2.value} USDT`
      )
    } else if (Number(newVal) > withdrawAmountMax2.value) {
      Toast(
        t(`提款金额不得高于最大提款限额`) + `: ${withdrawAmountMax2.value} USDT`
      )
    }
  }
})
...
const withdrawType = ref(t('银行卡'))
const withdrawTypeVal = ref('bank')
