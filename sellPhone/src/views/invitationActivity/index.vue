    ...
    <div class="block-content info">
      ...
      <div class="intro-table">
        ...
        <div v-for="(item, index) in inviteData" :key="index" class="item">
          <div
            v-if="index < inviteData.length - 1"
          >{{ item[1] }}-{{ Number(inviteData[index + 1][1]) - 1 }}</div>
...
const contentBg = ref(
  new URL('@/assets/imgs/me/invite-info-bg.png', import.meta.url)
)
...
nextTick(() => {
  ...
  getSysparaAction('mall_first_invite_recharge_rewards').then((res) => {
    const data = res.mall_first_invite_recharge_rewards
      ? JSON.parse(res.mall_first_invite_recharge_rewards)
      : []
    inviteData.value = data
  })

  // 有效充值金额
  getSysparaAction('valid_recharge_amount_for_first_recharge_bonus').then(
    (res) => {
      validAmount.value =
        res.valid_recharge_amount_for_first_recharge_bonus || 0
  ...
  sellerPromotional().then((res) => {
    inviteLink.value =
      res.download && res.code
        ? `${res.download}/#?usercode=${res.code}&lang=${locale.value}`
        : ''
  })
})
</script>
