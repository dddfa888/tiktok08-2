<template>
  <div class="recharge">
    <van-nav-bar
      safe-area-inset-top
      fixed
      :title="itemName == 'Hive' ? $t('提领') : $t('提现')"
      left-arrow
      @click-left="onClickLeft"
      :right-text="$t('记录')"
      @click-right="onClick1"
    />
    ...
    <template v-if="selectValue == 0">
      ...
      <div class="shuru">
        <el-select v-model="selectCurrency" :placeholder="$t('请选择')" @change="selectCurrencyChange">
          <el-option
            v-for="item of withdrawalMethod"
            :key="item.label"
            :label="item.label"
            :value="item.value"
          ></el-option>
      ...
      <div class="duoxuan">
        <div
          v-for="(item, indx) in blockchainNetwork"
          :key="indx"
          class="xuan1"
          :class="currentChainName === item ? 'xuanzhong' : 'weixuan'"
          @click="tongdao_qiehuan(item)"
        >
          {{ item }}
          <van-icon
            class-prefix="icon"
            v-if="currentChainName == item"
            name="xuanzejiaobiao"
      ...
      <div class="shuru">
        <el-input
          :placeholder="$t('请输入地址')"
          :disabled="allowEditWithdrawAddress"
          v-model="it.input2"
          clearable
        ></el-input>
      </div>
    ...
    <template v-if="selectValue == 1">
      ...
      <div class="shuru">
        <el-input :placeholder="$t('请输入开户行')" v-model="card.bankName" clearable></el-input>
      </div>
      <div class="biaoti" style="margin-bottom: 5px;">{{ $t('姓名') }}</div>
      <div class="shuru">
        <el-input :placeholder="$t('请输入姓名')" v-model="card.bankUserName" clearable></el-input>
      </div>
      <div class="biaoti" style="margin-bottom: 5px;">{{ $t('卡号') }}</div>
      <div class="shuru">
        <el-input :placeholder="$t('请输入卡号')" v-model="card.bankCardNo" clearable></el-input>
      </div>
    </template>
    <template v-if="selectValue == 2">
      ...
      <div class="shuru">
        <el-input :placeholder="$t('请输入账号')" v-model="card.bankCardNo" clearable></el-input>
      </div>
    </template>
    ...
    <div class="biaoti current_price">
      ...
      <span
        v-if="selectValue == 0"
      >≈ {{ exchangeRateConversion }} {{ selectCurrency }}</span>
    </div>
    <div class="info-tips">
      <p>
    ...
    <van-overlay :show="bindShow">
      <div class="bind_wrap">
        ...
        <div class="shuru">
          <el-select
            v-model="bindData.coin"
            :placeholder="$t('请选择取款方式')"
            @change="handleCoinChange"
          >
            <el-option
              v-for="item in withdrawalMethod"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
        ...
        <div class="shuru">
          <el-select
            v-model="bindData.blockchain_name"
            :placeholder="$t('请选择链接协议')"
            :no-data-text="$t('请先选择取款方式')"
          >
            <el-option
              v-for="item in linkProtocolOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
        ...
        <div class="shuru">
          <el-input :placeholder="$t('请输入钱包地址')" v-model="bindData.channel_address" clearable></el-input>
        </div>
        <div class="hint">{{ $t('仅能绑定一个收款地址!') }}</div>
        <van-button
          @click="requestHandleBind"
          class="submit_btn"
          block
          type="primary"
        >{{ $t('确认绑定') }}</van-button>
  ...
  methods: {
    ...
    biaodan_tijiao2() {
      ...
      if (this.selectValue == 0) {
        ...
        if (this.card.bankUserName.trim() === "")
          return Toast(this.$t("请输入姓名"));
        if (this.card.bankCardNo.trim() === "")
          return Toast(this.$t("请输入账号"));
      } else if (this.selectValue == 3) {
        if (this.card.bankCardNo.trim() === "")
          return Toast(this.$t("请输入账号"));
      } else {
        // 银行卡
        if (this.card.bankName.trim() === "")
          return Toast(this.$t("请输入开户行"));
        if (this.card.bankUserName.trim() === "")
          return Toast(this.$t("请输入姓名"));
        if (this.card.bankCardNo.trim() === "")
          return Toast(this.$t("请输入卡号"));
      }
        ...
        if (res.safeword == 1) {
          ...
          if (this.selectValue == 0) {
            ...
            sessionStorage.setItem(
              "withdraw_bankUserName",
              this.card.bankUserName
            );
    ...
    zhifutongdao() {
      zhifutongdao_post({}).then(res22 => {
        ...
        const tempItem = this.linkProtocol.filter(
          item => item.coin == this.selectCurrency
        );
        tempItem.forEach(item => {
    ...
    afterRead(file) {
      ...
      formData.append("file", file.file);
      tupianshangchuan_post(formData)
        .then(res => {
    ...
    handleInput(val) {
      if (val > this.userMoney?.money) {
        ...
        this.it.input1 = this.it.input1
          .replace(".", "$#$")
          .replace(/\./g, "")
          .replace("$#$", "."); // 将第一个小数点替换为$#$，去除所有小数点，再将$#$替换回小数点
        if (["BTC", "ETH"].includes(this.selectCurrency)) {
          this.it.input1 = this.it.input1.replace(
            /^(\-)*(\d+)\.(\d{0,10}).*$/,
            "$1$2.$3"
          ); // 最多允许输入10位小数
          this.it.input1 = this.it.input1.replace(
