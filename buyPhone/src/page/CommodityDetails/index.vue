<template>
  <div class="CommodityDetails">
    ...
    <div class="banner-product">
      ...
      >
        <van-swipe-item class="swipe-item" v-for="(item, index) in imageList" :key="index">
          <img
            :src="item"
            @click="ImagePreview({ images: imageList, startPosition: index })"
            class="img"
          />
        </van-swipe-item>
        <template #indicator>
    ...
    <div class="product-info-wrap">
      <div class="product-name">{{ goods_info.name }}</div>
      <div class="price-box">
        ...
        <div class="product-sold">{{ $t("销量") }} {{ priceFormatInt(goods_info.soldNum) }}</div>
      </div>
      <div class="sku_box" v-if="goods_info?.canSelectAttributes?.goodAttrs.length">
        <div class="attributes_box">
          <div
            class="attributes"
            v-for="(item, i) of goods_info?.canSelectAttributes?.goodAttrs"
            :key="i"
          >
            <div class="p_title" v-if="item?.attrName">{{ currentSkuTitle[i] }}</div>
            <ul v-if="item?.attrName">
              <li
                :class="[sub.attrValueId == activeAttrs[i] && 'active', sub?.icon && 'img_box', sub.disabled && 'disabled']"
                @click="handleToggle(sub, i, sub.disabled)"
                v-for="(sub, j) of item.attrValues"
                :key="j"
              >
      ...
      <div class="product-line-item">
        ...
        <div
          class="right"
          @click="isShippingShow = true"
        >${{ goods_info.goodsTax ? priceFormat(goods_info.goodsTax) : "0.00" }}</div>
      </div>
    ...
    <div class="Evaluation">
      ...
      <div class="nav-item" @click="jumpAllReviews">
        <div class="left" v-if="itemName != 'Inchoi'">
          {{ $t("用户评价") }}({{ evaluationNum > 99 ? '99+' : evaluationNum
          }})
        </div>
      ...
      <div class="sort">
        ...
        <span :class="[isEn && 'en_span', sort_index === 1 && 'active']" @click="sort_index = 1">
          {{ $t('好评') }}
        ...
        <span :class="[isEn && 'en_span', sort_index === 3 && 'active']" @click="sort_index = 3">
          {{ $t('差评') }}
        ...
        <div v-for="(item, index) in EvaluationInfo" :key="index" class="sheet-item">
          ...
          <div class="comment_image">
            <div v-for="(url, i) of item.images" :key="i">
              <img
                @click="ImagePreview({ images: item.images, startPosition: i })"
                v-if="url"
                :src="url"
                alt
              />
      ...
      <div class="centent">{{ EvaluationInfo.content }}</div>
    </div>
    <div class="details-wrap">
        ...
        <div class="shop-info">
          <div class="left">
            ...
            <div class="info">
              ...
              <div
                class="title"
              >{{ $t("好评率") }}: {{ Math.floor(SellerInfo.highOpinion * 100) || 100 }}%</div>
            </div>
          </div>
          <div class="right" @click="jumpShop">{{ $t("访问商店") }}</div>
        </div>

        <div class="shop-bottom">
          <div class="item">
            <div class="text">{{ goods_info.seller ? goods_info.seller.sellerGoodsNum : 100 }}</div>
            <div class="title">{{ $t("所有产品") }}</div>
          </div>
          <div class="item">
            <div
              class="text"
            >{{ (+goods_info.seller?.fake || 0) + (+goods_info.seller?.focusNum || 0) }}</div>
            <div class="title">{{ $t("关注") }}</div>
          </div>
          <div class="item">
            <div
              class="text"
            >{{ (+goods_info.seller?.soldNum || 0) + (+goods_info.seller?.fakeSoldNum || 0) }}</div>
      ...
      <div class="recommended_product" v-if="recommendedGoods?.length">
        ...
        <ul>
          <li
            v-for="(item, index) of recommendedGoods"
            :key="index"
            @click="getSellerGoodsInfo(item.id)"
          >
            <div class="img">
      ...
      <div class="shop-description-title" id="details">{{ $t("产品描述") }}</div>
      ...
      <div class="may_like" v-if="likeGoods?.length">
        ...
        <div class="ml_list">
          ...
          >
            ...
            <div class="text padd-15">{{ $t("销量") }} {{ priceFormatInt(item.soldNum) }}</div>
      ...
      <div class="fiexd-left">
        ...
        <div class="item" @click="isCard = productPropertiesShow = true">
          <div class="icon gouwuc-icon">
            <div class="tips" v-if="$ls.get('productList')">{{ $ls.get('productList').length }}</div>
          </div>
    ...
    <van-popup closeable v-model="isShippingShow" position="bottom" :style="{ height: '35%' }">
      <div class="pop-header-title">{{ $t("运费说明") }}</div>
      <div class="centent-wrap">
        <p>{{ $t("1、跨境商品运费构成：运费=派送费+长途运费+送货费;") }}</p>
        <p>{{ $t("2、如不满足包邮条件，按实际收取运费产品") }}</p>
        <p>{{ $t("3、最终解释权归平台所有；") }}</p>
      </div>
    </van-popup>
      ...
      <div class="card-wrap">
        <div class="left">
          ...
          <div class="goods_info">
            ...
            <div
              class="money"
            >${{ priceFormat(goods_info.discountPrice) || priceFormat(goods_info.sellingPrice) }}</div>
          </div>
        </div>
        <div class="right">
          <van-stepper :disable-input="true" :max="maxBuyNum" integer v-model="bugNum" />
    ...
    <!-- 产品属性 -->
    <van-popup
      v-model="productPropertiesShow"
      position="bottom"
      closeable
      round
      class="product_properties"
    >
      <div class="cart_title" v-if="isCard">{{ $t("添加购物车") }}</div>
      <div class="p_top">
        <div class="img">
          <img
            :src="currentCoverImg || goods_info.imgUrl1 || $defaultGoodsImage"
            @click=" ImagePreview([currentCoverImg || goods_info.imgUrl1])"
            alt
          />
        </div>
        <div class="t_right">
          ...
          <div class="num_box">
            <div>
              <!-- <p></p> -->
              {{ $t("已选") }}:
              <span v-for=" item  of  currentSkuTitle " :key="item">{{ item }}</span>
            </div>
...
export default {
  ...
  created() {
    ...
    // 获取中评差评数量
    apiEvaluationCountType({ goodId: this.$route.query.sellerGoodsId }).then(
      res => {
        const {
          havePicture,
          mediumReview,
  ...
  methods: {
    ...
    getSkusAndPrice(find, arr) {
      ...
      if (skuMap[attrIds] && !find) {
        ...
        this.goods_info.discountPrice =
          skuMap[attrIds].discountPrice || skuMap[attrIds].sellingPrice;
        this.currentSkuId = skuMap[attrIds].skuId;
        this.currentCoverImg = skuMap[attrIds].coverImg;
        this.imageList = skuMap[attrIds].img;
        this.btnDisabled = false;
        this.originalPrice = skuMap[attrIds].discountPrice
          ? skuMap[attrIds].price
          : 0;
      } else {
        ...
        if (this.$ls.get("productList")) {
          ...
          try {
            productList.forEach(item => {
              if (
                item.id == this.goods_info.id &&
                item.skuId == this.goods_info.skuId
              ) {
    ...
    getSellerGoodsInfo(sellerGoodsId) {
      ...
      getSellerGoods(dataJson).then(data => {
        ...
        if (data.canSelectAttributes?.goodAttrs?.length) {
          ...
          const ids = data.canSelectAttributes.goodAttrs.map(
            item => item.attrId
          );
          this.allSkus.forEach(item => {
            item.attrs = item.attrs.sort(
              (star, next) =>
                ids.indexOf(star.attrId) - ids.indexOf(next.attrId)
            );
          });
        }
        for (let i = 0; i < 10; i++) {
  ...
  .buy-bottom-fiexd {
    ...
    .fiexd-left {
      ...
      .item {
        ...
        .collect-icon {
          background: url("../../assets/image/commodity/Star6.png") center
            no-repeat;
          background-size: 100% 100%;
        }

        .collect-check-icon {
          background: url("../../assets/image/commodity/Star7.png") center
            no-repeat;
          background-size: 100% 100%;
        }

        .kefu-icon {
          background: url("../../assets/image/commodity/kefu.png") center
            no-repeat;
          background-size: 100% 100%;
        }
        ...
        .gouwuc-icon {
          background: url("../../assets/image/commodity/gouwuc.png") center
            no-repeat;
          background-size: 100% 100%;
        }
      }
