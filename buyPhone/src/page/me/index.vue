  ...
  <div class="my-container" style="min-height: 100vh;">
    ...
    <div class="my-enter-content">
      <div class="balance-content" v-if="isShelves">
        ...
        <div v-else class="money">{{ userMoney && priceFormat(userMoney.money) || '0.00' }}</div>
        <div class="btn">
          <div
            class="item"
      ...
      <div
        ...
        :style="{ 'background-image': 'url(' + images.bar + ')' }"
        @click="merchantJump()"
      >
        <div>
...
export default {
  ...
  methods: {
    ...
    toCustomerService() {
      ...
      //   title: this.$t("联系客服"),
      //   message: `
      //   <div id="customerServiceWrapper" style="display:flex;justify-content:space-around;">
      //     <div id="meiqia" style="cursor:pointer;">${this.$t("客服")}1</div>
      //     <div id="sitou" style="cursor:pointer;">${this.$t("客服")}2</div>
      //   </div>
      // `,
      ...
      // setTimeout(() => {
      //   const meiqiaEl = document.getElementById("meiqia");
      //   const sitouEl = document.getElementById("sitou");

      //   if (meiqiaEl) {
      //     meiqiaEl.addEventListener("click", () => {
      //       console.log("点击美洽客服");
      //       Dialog.close(); // 关闭弹窗
      apiGetCustomerService().then(res => {
        if (res.customer_service_url) {
      ...
      //   if (sitouEl) {
      //     sitouEl.addEventListener("click", () => {
      //       console.log("点击思途客服");
      //       Dialog.close(); // 关闭弹窗
      //       if (self.$route.path !== "/service") {
      //         self.$router.push({ path: "/service" });
      //       } else {
      //         console.log("已经在 /service?id=123，跳转被忽略");
      //       }
      //     });
      //   }
      // }, 300);
