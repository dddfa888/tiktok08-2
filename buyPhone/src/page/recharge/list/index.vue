<template>
  <div class="rechargeList">
    ...
    <router-link
      to="/customerServiceIndex?path=customerService"
    ...
    <!-- <div v-for="item of threePartyRecharge" :key="item.name">
            <router-link :to="`/bank-card?type=${item.name}&min=${item.min}&max=${item.max}`"
                v-if="['Shop2u', 'EShop','Spar'].includes(itemName)">
                <div class="chongzhi-fs">
    ...
    <div v-for="item of withdrawalMethod" :key="item.label">
      <router-link
        :to="`/recharge?type=${item.label}`"
        v-if="['Shop2u', 'EShop','Spar','TikTokMall'].includes(itemName)"
      >
        <div class="chongzhi-fs">
