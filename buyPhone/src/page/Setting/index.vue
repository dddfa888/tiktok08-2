...
export default {
  ...
  created() {
    ...
    if (!isLogin()) {
      ...
      this.navData[1].path = this.userInfo.safeword
        ? "/FundPasswordReset"
        : "/FundPasswordSettings";

      //   this.navData[2].tips = this.userInfo.phoneverif ? '' : this.$t('未绑定')
      if (this.userInfo.phoneverif) {
  this.navData.splice(2, 1);
}
    ...
    if (
      ["Argos", "ArgosShop", "Inchoi", "EShop"].includes(this.itemName) ||
      this.isShelves
    ) {
      this.navData.push({
        name: this.$t("账号注销"),
