<template>
  ...
  <div :class="['header',isStore&&'no-gap']">
    <div
      class="header-fiexd"
      :style="!headerFlag?'padding:0 !important; transition: padding .5s linear 0s; background: rgba(255, 255, 255, 0.95); box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); height:auto !important;':''"
    >
      <div class="header-top flex-between app-container">
        <div
          class="header-logo flex-start"
          @click="goto('/')"
          :style="itemname == 'TikTok' ? 'height:78px': itemname == 'SM-wholesale shop'? 'width:220px':'' "
        >
          <img
            :src="itemname =='TikTok'?  require(`@/assets/image/${itemname}/shoplogo.png`): itemname =='Spar' ? require(`@/assets/image/${itemname}/${itemname}logo.png`):require(`@/assets/image/${itemname}/shoplogo.svg`)"
            :style="itemname == 'SM-wholesale shop'? 'width:220px;margin-right:auto':itemname =='TikTok'?'height:38.2px ;width:160px ':itemname =='Spar'?'width:70px': 'width:50px'"
          />
        </div>
        ...
        <div class="header-user">
          <div class="flex-start">
            ...
            <div class="no-login" v-if="!userInfo.username">
              <span class="text" @click="goto('/login')">{{ $t("message.home.login" /*登录 */) }}</span>
              <span>{{ $t("message.home.or") }}</span>
              <span
                class="text"
                @click="goto('/register')"
              >{{ $t("message.home.register" /**注册 */) }}</span>
            </div>
