	...
	public Page pagedQuery(int pageNo, int pageSize, String name_para, String rolename, String checkedPartyId, Boolean online, String loginIp_para, String phone, String agentPartyId,String rmk) {
		...
		StringBuffer queryString = new StringBuffer(
				" SELECT party.UUID id, party.NAME name, party.USERNAME username, party.PHONE phone, party.EMAIL email,"
						...
						+ " party_parent.USERNAME username_parent, party.LOGIN_IP login_ip, party.GIFT_USER gift_user, party.USER_LEVEL user_level, "
						+ " party.REGSITER_USERCODE register_usercode, ROUND(wallet.MONEY,2) money ,wallet.FROZEN_STATE frozen_state,"
						+ " ROUND(wallet.MONEY_AFTER_FROZEN,2) moneyAfterFrozen ,ROUND(wallet.PURCHASE_AMOUNT,2) purchase "
		);
		queryString.append(
				" FROM PAT_PARTY party LEFT JOIN PAT_USER_RECOM user ON user.PARTY_ID = party.UUID "
						+ " LEFT JOIN PAT_PARTY party_parent ON user.RECO_ID = party_parent.UUID "
...
//			queryString.append("AND partyId is not null AND LENGTH(trim(partyId))!=0");
	 ...
	 */
	@Override
	public void updatePurchase(String opName, String opSafeWord, String opIP, String toID, String type, BigDecimal money) {
		//验证资金密码
		...
		double before=wallet.getPurchaseAmount();
		double after=0.0;
		String op="添加";
		// 账变日志
		MoneyLog moneyLog = new MoneyLog();
		...
		//操作日志
		Log log = new Log();
		if("subPurchase".equals(type)){//添加
			op="减少";
			money=money.negate();
		}
		after=money.add(BigDecimal.valueOf(before)).doubleValue();
		if(after<0){
			after=0.0;
		}
		wallet.setPurchaseAmount(after);
		this.walletService.update(wallet);//更新钱包
		moneyLog.setContent_type(Constants.MONEY_LOG_CONTENT_PURCHASE);
		moneyLog.setFreeze(0);
		moneyLog.setCategory(Constants.MONEY_LOG_CATEGORY_PURCHASE);
		moneyLog.setAmount_before(before);
		moneyLog.setAmount(money.doubleValue());
		moneyLog.setAmount_after(after);
		moneyLog.setPartyId(toID);
		moneyLog.setWallettype(Constants.WALLET);
		this.moneyLogService.save(moneyLog);
		...
		log.setLog("ip:"+opIP+",管理员"+op+"采购资金，修改数量[" + money + "]");
		this.logService.saveSync(log);

		walletLog.setCategory(Constants.MONEY_LOG_CATEGORY_PURCHASE);
		walletLog.setPartyId(toID);
		walletLog.setOrder_no("");
		walletLog.setStatus(1);
		walletLog.setAmount(money.doubleValue());
		walletLog.setUsdtAmount(money.doubleValue());
		walletLog.setWallettype(Constants.WALLET);
		this.walletLogService.save(walletLog);
	}
