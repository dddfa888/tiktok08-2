...
//		page.setElements(format(page.getElements()));
	...
	@Override
	public Page userReports(int pageNo, int pageSize,String startTime,String endTime,String loginPartyId,
							String usernameOrUid,String targetPartyId,String sortColumn,String sortType,
							String sellerId ,String sellerName, String all_para_party_id) {
		Map<String,Object> parameters = new HashMap<>();
		StringBuffer queryString = new StringBuffer();
		queryString.append("SELECT party.ROLENAME AS rolename,party.USERNAME AS username,party.REMARKS AS remarks," +
				" party.USERCODE AS UID ,party.UUID AS partyId,IFNULL(wallet.MONEY,0) AS money,");//用户
		queryString.append("IFNULL(SUM(ud.RECHARGE),0) AS recharge,IFNULL(SUM(ud.RECHARGE_USDT),0) AS recharge_usdt," +
				"IFNULL(SUM(ud.RECHARGE_ETH),0) AS recharge_eth,IFNULL(SUM(ud.RECHARGE_BTC),0) AS recharge_btc," +
				"IFNULL(SUM(ud.RECHARGE_HT),0) AS recharge_ht,IFNULL(SUM(ud.RECHARGE_LTC),0) AS recharge_ltc," +
				"IFNULL(SUM(ud.RECHARGE_USDT),0) AS recharge_usdt,IFNULL(SUM(ud.WITHDRAW),0) AS withdraw," +
				"IFNULL(SUM(ud.WITHDRAW_ETH),0) AS withdraw_eth,IFNULL(SUM(ud.WITHDRAW_BTC),0) AS withdraw_btc,"
				+ "IFNULL(SUM(ud.RECHARGE_WITHDRAWAL_FEE),0) AS recharge_withdrawal_fee," +
				"IFNULL(SUM(ud.GIFT_MONEY),0) AS gift_money,IFNULL(SUM(ud.RECHARGE)-SUM(ud.WITHDRAW),0) AS balance_amount, ");//充提
		queryString.append("IFNULL(SUM(ud.AMOUNT),0) AS amount,IFNULL(SUM(ud.FEE),0) AS fee," +
				"IFNULL(SUM(ud.ORDER_INCOME),0) AS order_income, IFNULL(SUM(ud.RECHARGE_COMMISSION),0) AS rechargeCommission,");//永续
		queryString.append("IFNULL(SUM(ud.FINANCE_AMOUNT),0) AS finance_amount,IFNULL(SUM(ud.FINANCE_INCOME),0) AS finance_income, " +
				"IFNULL(SUM(ud.RECHARGE_COMMISSION),0) AS rechargeCommission, IFNULL(SUM(ud.WITHDRAW_COMMISSION),0) AS withdrawCommission,");//理财
		queryString.append("IFNULL(SUM(ud.EXCHANGE_AMOUNT),0) AS exchange_amount,IFNULL(SUM(ud.EXCHANGE_FEE),0) AS exchange_fee," +
				"IFNULL(SUM(ud.EXCHANGE_INCOME),0) AS exchange_income,IFNULL(SUM(ud.COIN_INCOME),0) AS coin_income, ");//币币
		queryString.append("IFNULL(SUM(ud.FURTURES_AMOUNT),0) AS furtures_amount,IFNULL(SUM(ud.FURTURES_FEE),0) AS furtures_fee," +
				"IFNULL(SUM(ud.FURTURES_INCOME),0) AS furtures_income, ");//交割
		queryString.append("IFNULL(SUM(ud.MINER_AMOUNT),0) AS miner_amount,IFNULL(SUM(ud.MINER_INCOME),0) AS miner_income, ");//矿机
		queryString.append("IFNULL(SUM(ud.THIRD_RECHARGE_AMOUNT),0) AS third_recharge_amount, ");//三方充值
		queryString.append("IFNULL(SUM(ud.EXCHANGE_LEVER_AMOUNT),0) AS exchange_lever_amount," +
				"IFNULL(SUM(ud.EXCHANGE_LEVER_FEE),0) AS exchange_lever_fee,IFNULL(SUM(ud.EXCHANGE_LEVER_ORDER_INCOME),0) AS exchange_lever_order_income, ");//币币杠杆
		queryString.append("IFNULL(SUM(ud.REBATE_1),0) AS rebate1, IFNULL(SUM(ud.REBATE_2),0) AS rebate2,  " +
				"IFNULL(sum(ud.REBATE_1) + sum(ud.REBATE_2),0) rebateLump,  ");//佣金
		queryString.append("IFNULL(SUM(ud.TRANSLATE),0) AS translate, IFNULL(SUM(ud.SELLER_TOTAL_SALES),0) AS sellerTotalSales," +
				" IFNULL(SUM(ud.SELLER_COMMISSION),0) AS sellerCommission, ");//佣金
...
//			objs[i++] = data.get("furtures_amount");
	...
	public Map<String, Object> queryWillIncomeBySellerIds(List<String> sellerIds , String startTime , String endTime ) {
		...
		StringBuilder sql = new StringBuilder("select SELLER_ID as sellerId, " +
				"cast((SUM(IFNULL(SYSTEM_PRICE,0)) + SUM(IFNULL(PROFIT,0))) AS DECIMAL (19, 2)) AS willIncome  " +
				"FROM T_MALL_ORDERS_PRIZE  WHERE PURCH_STATUS =1 AND PROFIT_STATUS = 0  " +
				"AND PURCH_TIME_OUT_STATUS=0 AND SELLER_ID in (:sellersList)  ");
	...
	@Override
	public Map<String, Object> queryRebateBySellerIds(List<String> sellerIds , String startTime , String endTime ) {
		...
		StringBuilder sql = new StringBuilder("select SELLER_ID as sellerId, " +
				"CAST(SUM(IFNULL(PROFIT, 0)) AS DECIMAL(19, 2)) AS rebate FROM T_MALL_ORDERS_PRIZE  " +
				"WHERE PURCH_STATUS =1 AND PROFIT_STATUS = 1  AND SELLER_ID in (:sellersList)  ");
		...
		for (Map<String, Object> data : maps) {
			result.put(data.get("sellerId").toString(), data.get("rebate"));
		}
		return result;
	}
