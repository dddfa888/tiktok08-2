...
<form action="<%=basePath%>/mall/seller//freezeSeller.action"
	  method="post" id="succeededForm44">
	<input type="hidden" name="pageNo" id="pageNo" value="${pageNo}">
	<input type="hidden" name="seller_Id44" id="seller_Id44" value="${seller_Id44}">
</form>

<%--解冻店铺--%>

<form action="<%=basePath%>/mall/seller//unFreezeSeller.action"
	  method="post" id="succeededForm45">
	<input type="hidden" name="pageNo" id="pageNo" value="${pageNo}">
	<input type="hidden" name="seller_Id45" id="seller_Id45" value="${seller_Id45}">
</form>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal"
									aria-hidden="true"></button>
							<h4 class="modal-title">当前累计采购金额</h4>
						</div>
			...
			success : function(data) {
				...
				if (data.code === 200) {
					...
					for (var key in levels) {
						...
						selectHtml += '<option value="' + key + '" ' + selected + '>' + key + '(累计采购：≥' + levels[key] + ')'  + '</option>';
					}
