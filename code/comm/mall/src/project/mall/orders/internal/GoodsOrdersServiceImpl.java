...
//        return orders;
    ...
    private MallOrdersPrize createOneOrder(Party party, MallAddress address, String sellerId,
        ...
        int i = 0;
        int goodsType = 0;
        Double sfee = sysparaService.find("order-freight").getDouble();
...
//            walletService.update(wallet.getPartyId().toString(), Arith.sub(0, prize));
            ...
            moneyLog.setLog("支付[" + list.size() + "]个订单"+ list.get(0).getId().toString());
            moneyLog.setPartyId(partyId);
            moneyLog.setWallettype(Constants.WALLET);
            moneyLog.setContent_type(Constants.MONEYLOG_CONTNET_PAY_ORDER);
    ...
    public void updateReceiptOrders(String partyId, String orderId) {
        try {
            ...
            order.setStatus(4);
            order.setUpTime(this.getProfitTime());
            getHibernateTemplate().update(order);
...
//            如果有折扣，按照采购折扣价格计算
            ...
            Double push = prize;
            prize = Arith.add(prize, order.getSellerFees());
//            Wallet wallet = walletService.saveWalletByPartyId(partyId); 这里经常出现缓存不一致的问题，改为直接查库
            ...
            double purchaseAmount = wallet.getPurchaseAmount();// 采购资金
            if (purchaseAmount > 0) {// 采购优先使用采购资金
                amount_before += purchaseAmount;
                purchaseAmount = purchaseAmount - prize;
                if (purchaseAmount >= 0) {
                    prize = 0.0;
                    wallet.setPurchaseAmount(purchaseAmount);
                } else {
                    prize -= wallet.getPurchaseAmount();
                    wallet.setPurchaseAmount(0.0);
                }
            }
            if (amount_before < prize) {
                throw new BusinessException("余额不足");
...
//            walletService.update(wallet.getPartyId().toString(), Arith.sub(0, prize));
            ...
            moneyLog.setCategory(Constants.MONEYLOG_CATEGORY_COIN);
            moneyLog.setAmount_before(amount_before);
            moneyLog.setAmount(Arith.sub(0, push+order.getSellerFees()));
            if (wallet.getFrozenState() == 0) {
                moneyLog.setAmount_after(wallet.getMoney() + wallet.getPurchaseAmount());
                moneyLog.setFreeze(0);
            } else {// 处于冻结状态时
                moneyLog.setAmount_after(wallet.getMoneyAfterFrozen() + wallet.getPurchaseAmount());
                moneyLog.setFreeze(1);
            }
...
//            redisHandler.remove(lockKey);
    ...
    public Integer updateAutoReceipt() {
        ...
        if (CollectionUtils.isNotEmpty(lists)) {
            ...
            Map<String, Object> params = new HashMap<>();
            params.put("orderList", orderList);
            params.put("recTime", this.getProfitTime());
...
//    }
    ...
    @Override
    public void updateProfit(String orderId) {
    ...
    @Override
    public void updateProfitQueue(String orderId,String admin) {
        ...
        int profitStatus = order.getProfitStatus();

        try {
            if ((status == 4 || status == 5) && profitStatus == 0) {
                log.info("订单[{}][status:{}][profit-status:{}]添加释放佣金队列! 操作人:{}", orderId, status, profitStatus,admin);
                    order.setUpTime(this.getProfitTime());
                    this.getHibernateTemplate().update(order);
            }
        } catch (Exception e) {
            logger.error("添加释放佣金队列异常", e);
        } finally {
            redisHandler.remove(lockKey);  // 释放订单锁
        }
    }
    ...
    long getProfitTime(){
        Double mall_autoProfit = sysparaService.find("mall_autoProfit").getDouble();
        return System.currentTimeMillis() - (long) (60 * 60 * 1000 * mall_autoProfit) - 1000;
    }
    @Override
...
//        return baseTraffic * (autoStart + (int) (Math.random() * (autoEnd - autoStart + 1)));
    ...
    public Map<String, Object> findBySellId(String sellId) {
        ...
        if (StringUtils.isNotEmpty(sellId)) {
            sql.append(" where IS_VALID=1 AND  goods.SELLER_ID = ? ");
    ...
    public void updateWalletWithLock(String orderId, String partyId, String type, BigDecimal principal, BigDecimal amount) {
        ...
        try {
            ...
            try {
                ...
                logger.error("钱包日志写入异常 已有日志:订单: " + orderId);
            }
