...
//				return Page.EMPTY_PAGE;
    ...
    public void updateFreedCol(String[] ids, String admin) {
            for (String id : ids) {
                ...
                if (orders.getStatus() >= 4 && orders.getStatus() <= 5 && orders.getProfitStatus() == 0 && orders.getReturnStatus() == 0) {
                    goodsOrdersService.updateProfitQueue(orders.getId().toString(), admin);
    ...
    @Override
    public void updateReceiptCol(String[] ids,String adminName) {
        for (String id : ids) {
            MallOrdersPrize orders = this.findById(id);
            if (Objects.isNull(orders)) {
                continue;
            ...
            if (orders.getStatus() == 3) {
                orders.setStatus(4);
                goodsOrdersService.updateProfitQueue(orders.getId().toString(), adminName);
                this.getHibernateTemplate().update(orders);
