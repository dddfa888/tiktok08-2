<template>
  <div class="info_page_container" style="width: 480px">
    ...
    >
      <div>
        <el-form-item :label="$t('message.home.coinBlock')">
          <el-select
            ...
            @change="changeCoinType(coinType)"
          >
            <el-option
      ...
      >
        <el-input
          ...
          :placeholder="$t('message.home.withdrawInputTips' /** 请输入提币地址*/)
          "
          :disabled="swithButton"
        />
        <span
      ...
      <el-form-item>
        <el-button
          ...
          type="primary"
          @click="withdraw"
        >{{ $t("message.home.btnSure" /**确认 */) }}</el-button>
      </el-form-item>
    </el-form>
    <el-form
    ...
    <el-form
      ref="withdrawZfbForm"
      :model="formZfb"
      v-if="rachargeTypeV === 'zfb' && itemname !== 'FamilyShop' && itemname !== 'SM-wholesale shop'"
      :rules="zfbRules"
    >
      <el-form-item :label="$t('message.home.actualName')" prop="bankUserName">
        <el-input
          v-model="formZfb.userName"
          @keypress.native="onKeyPress"
          :placeholder="$t('message.home.yourName')"
        />
      </el-form-item>
      <el-form-item :label="$t('message.home.支付宝账号')" prop="bankCardNo">
        <el-input
          v-model="formZfb.bankCardNo"
          :placeholder="$t('message.home.pleaseEnterAccount')"
        />
      </el-form-item>
      <!-- 金额 -->
      <el-form-item :label="$t('message.home.quantity')" prop="quantity">
        <el-input
          v-model="formZfb.quantity"
          maxlength="12"
          onkeyup="if(isNaN(value))execCommand('undo')"
          onafterpaste="if(isNaN(value))execCommand('undo')"
          :disabled="money < 1"
      ...
      <el-form-item>
        <div
          style="color:#56B069"
        >{{ $t('message.home.当前余额') }}: {{ numberFormat(money) }} USDT ≈ {{ numberFormat($big(money).div(fee)) }} USDT</div>
      </el-form-item>
      <el-form-item>
        <div class="withdraw-tips flex-between">
          <span>
    ...
    <el-form
      ref="withdrawWxForm"
      :model="formWx"
      v-if="rachargeTypeV === 'wx' && itemname !== 'FamilyShop' && itemname !== 'SM-wholesale shop'"
      :rules="wxRules"
    >
      <!-- <el-form-item :label="$t('message.home.actualName')" prop="bankUserName">
        <el-input
      ...
      <el-form-item
        style="align-items: start;display: flex;flex-direction: column;"
        :label="$t('message.home.上传微信收款码（需保存收款码到相册)')"
        prop="qdCode"
      >
        <el-upload
...
export default {
  ...
  data() {
    return {
      ...
      bankRules: {
        ...
        bankUserName: [
          common.ruleUtils.getRule("required", this.$t("message.home.yourName"))
        ],
        bankCardNo: [
          common.ruleUtils.getRule(
            "required",
      ...
      zfbRules: {
        ...
        quantity: [
          ...
          common.ruleUtils.getRule("validator", "", {
            validator: (rule, value, callback) => {
              const max = this.$big(this.money).div(this.fee)
                ? this.$big(this.money).div(this.fee)
                : 10e8;
              if (Number(value) > max) {
                callback(this.$t("message.home.validatorMaxNumber"));
              }
  ...
  watch: {
    ...
    "form.quantity": {
      handler(newValue, oldValue) {
        if (newValue !== oldValue) {
          const amount = accTimes(newValue, accMinus(1, this.withdrawFee ?? 0));
  ...
  methods: {
    ...
    async getwithdraw() {
      ...
      if (res.data.openWithdrawAddressBinding) {
        if (!res.data.existWithdrawAddress && !res.data.chainName) {
    ...
    async payCallback(password, successCallBack, failCallBack) {
      ...
      this.requesWithdraw({
        ...
        amount:
          this.form.quantity ||
          this.formBank.bankQuantity ||
          this.formZfb.quantity ||
          this.formWx.quantity,
        from: this.form.address,
        channel: this.coinType,
        bankUserName:
          this.formBank.bankUserName ||
          this.formZfb.userName ||
          this.formWx.userName,
        bankName: this.formBank.bankName,
        bankCardNo:
          this.formBank.bankCardNo ||
          this.formZfb.bankCardNo ||
          this.formWx.bankCardNo,
