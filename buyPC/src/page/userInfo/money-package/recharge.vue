  ...
  <div class="info_page_container" style="width: 480px">
    ...
    <el-form v-if="itemname == 'Shop2u' || itemname == 'SM-wholesale shop'">
      <el-form-item :label="$t('message.home.rechargeMethod')">
        <el-select style="width: 100%" v-model="rachargeTypeV" @change="changeType(rachargeTypeV)">
          <el-option
    ...
    >
      ...
      <el-form-item :label="$t('message.home.充值币种')" prop="coin">
        <el-select style="width: 100%" v-model="form.coin">
          ...
          <div
            ...
            v-if="itemname == 'FamilyShop' || itemname == 'Spar'"
            @click="openService"
          >{{ $t("message.home.银行卡") }}</div>
          <div
            :key="$t('message.home.支付宝')"
            class="el-select-dropdown__item"
            v-if="itemname == 'Spar'"
            @click="openService"
          >{{ $t("message.home.支付宝") }}</div>
          <div
            :key="$t('message.home.微信')"
            class="el-select-dropdown__item"
            v-if="itemname == 'Spar'"
            @click="openService"
          >{{ $t("message.home.微信") }}</div>
        </el-select>
      </el-form-item>
      <!-- <el-form-item :label="$t('message.home.充值币种')" prop="coin">
        <el-select style="width: 100%" v-model="form.coin">
      ...
      <el-form-item :label="$t('message.home.blockchainNetwork' /**区块链网络 */)" prop="blockId">
      ...
      <div class="qr-code">
        ...
        <el-button class="qr-btn" @click="saveImage">{{ $t("message.home.saveQr" /**保存二維碼 */) }}</el-button>
      </div>
      ...
      <el-form-item :label="$t('message.home.uploadTips')" class="image-uploader" prop="image">
        <el-upload
          :action="UploadApi"
      ...
      <el-form-item>
        <el-button
          ...
          :loading="btnLoading"
          @click="recharge"
        >{{ $t("message.home.btnSure") }}</el-button>
      </el-form-item>
    </el-form>
    <!-- 银行卡充值 -->
    <el-form
    ...
    >
      <el-form-item :label="$t('message.home.充值币种')" prop="coin">
        <el-select style="width: 100%" v-model="bankForm.frenchCurrency" @change="getcurrency">
          <el-option
    ...
    inputHandler(type, e) {
      if (e == "GCash2.0") {
        ...
        if (type !== "GCash" && e !== "GCash2.0") {
          ...
          if (
            this.gcashFrom.amount >= this.minAmount &&
            this.gcashFrom.amount <= this.maxAmount
          ) {
            this.gcashbtn = false;
          } else {
            this.gcashbtn = true;
