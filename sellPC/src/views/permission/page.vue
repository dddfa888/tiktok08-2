<template>
  <div class="app-container px-20">
      ...
      <div style="display: flex;justify-content: space-between;align-items: center;">
          ...
          <div class="flex items-center">
            ...
            <div class="ml-21">
              ...
              <p class="font-18" style="color: #999">
                ...
                <span>
                  ({{ $t('待到账金额') }}
                  <FormatNumberShow :data="huoquyue.frozenMoney" :currency="true" />)
                </span>
              </p>
              <p class="font-18" style="color: #999">
                <span>
                  ({{ $t('采购资金') }}
                  <FormatNumberShow :data="huoquyue.purchase" :currency="true" />)
                </span>
              </p>
            </div>
          </div>
        ...
        <div class="flex">
          <el-button
            type="primary"
            @click="rechargeEvent"
          >{{ ['HIVE'].includes(settings.projectTitle) ? $t('加值') : $t('充值') }}</el-button>
          <el-button
            type="primary"
            @click="withdrawEvent"
          >{{ ['HIVE'].includes(settings.projectTitle) ? $t('提领') : $t('提现') }}</el-button>
          <!--<el-button type="primary" @click="bankEvent">{{ $t('银行卡绑定') }}</el-button>
           <el-button type="primary" plain @click="intoPage" v-if="['HIVE'].includes(settings.projectTitle)">
          <el-button type="primary" plain @click="intoPage">{{ $t('信贷服务') }}</el-button>-->
        </div>
      </div>
    </el-card>
    <!--<el-carousel
    ...
    >
      <el-carousel-item
        v-if="userInfo&&userInfo.kyc_status == 2&&inviteFriends&&inviteFriends.length>0"
      >
          ...
          <div class="first-charge">
            ...
            <div class="first-charge-content">
              <div class="first-charge-text">
                ...
                <div class="first-charge-desc">
                  <span
                    v-for="(item,index) in syspara"
                    :key="index"
                  >{{ item.content }}{{ index !== syspara.length - 1 ? '，' : '' }}</span>
                </div>
              </div>
              <div class="first-charge-button">
                <el-button
                  ...
                  type="primary"
                  @click="receiveBonus"
                  v-if="!showReceiveBtn&&merchantInfo.rechargeBonusStatus===1"
                >{{ $t('领取') }}</el-button>
                <el-button
                  class="first-charge-btn"
                  type="primary"
                  @click="rechargeEvent"
                  v-else-if="!showReceiveBtn&&merchantInfo.rechargeBonusStatus===0"
                >{{ $t('去充值') }}</el-button>
                <el-button
                  class="first-charge-btn"
                  type="primary"
    ...
    <el-card>
      <div class="flex py-15">
        ...
        >{{ ['HIVE'].includes(settings.projectTitle) ? $t('加值') : $t('充值') }}</div>
        <div
          :class="{'btn-active': type === 'withdraw'}"
          class="button-item"
          @click="changeTab('withdraw')"
        >{{ ['HIVE'].includes(settings.projectTitle) ? $t('提领') : $t('提现') }}</div>
      </div>
      <!--  充值表格    -->
      <el-table
        :data="tableData"
      ...
      >
        ...
        <el-table-column :label="$t('订单号')" prop="order_no"></el-table-column>
        ...
        <el-table-column :label="$t('支付/凭证')" prop="state">
          <template slot-scope="{row}">
            ...
            <el-button size="mini" type="text" v-else-if="row.img" style="position: relative;">
              {{ $t('查看') }}
              <el-image
                class="image-mini"
                :src="row.img"
    ...
    <el-dialog :title="$t('银行卡绑定')" :visible.sync="dialogFormVisible">
      <el-form :model="formBank">
        ...
        <el-form-item :label="$t('姓名')" label-width="120">
          <el-input v-model="formBank.accountName" autocomplete="off"></el-input>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmBank">{{ $t('确认') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
