  ...
  <div class="withdraw-main">
          ...
          <template v-if="formWithdraw.withdrawalMethod === 1">
            <el-form-item :label="$t('提现币种')" prop="currencyType">
              ...
              <el-select
                ...
                :disabled="withdrawOpenData.openWithdrawAddressBinding === 1"
              >
                <el-option v-for="item in currencyType" :key="item" :label="item" :value="item"></el-option>
                <el-option
                  :label="$t('银行卡')"
                  v-if="['FamilyShop', 'JustShop'].includes(projectTitle)"
                  value="bankCard"
            ...
            <template v-if="!(formWithdraw.currencyType === 'bankCard')">
              <el-form-item :label="$t('提现网络')" prop="linkProtocol">
                ...
                <el-select
                  ...
                  :disabled="withdrawOpenData.openWithdrawAddressBinding === 1"
                >
                  <el-option v-for="item in linkProtocol" :key="item" :label="item" :value="item"></el-option>
                </el-select>
              </el-form-item>
              ...
              <el-form-item :label="$t('提现金额')" prop="account">
                <div style="position: relative">
                  ...
                  <el-tag size="mini" type="success" class="tip-tag">
                    <span>{{ $t('钱包余额') }}</span>&nbsp;
                    <span>
                      <FormatNumberShow :data="userBalance" :decimalPlaces="6" />USDT
                    </span>
                  </el-tag>
                  <el-tag
                    size="mini"
                  ...
                  <div class="doc-tag">
                    <el-tag
                      :type="!['USDT', 'usdt'].includes(formWithdraw.currencyType) ? '' : 'success'"
                      style="margin: 0 4px"
                    >{{ formWithdraw.currencyType }}</el-tag>
                  </div>
                </div>
              </el-form-item>
              <el-form-item :label="$t('实际到账')">
                <div style="position: relative">
                  ...
                  <div
                    ...
                    :type="!['USDT', 'usdt'].includes(formWithdraw.currencyType) ? '' : 'success'"
                  >
                    <el-tag style="margin: 0 4px">{{ formWithdraw.currencyType }}</el-tag>
                  </div>
                </div>
          ...
          <template v-if="formWithdraw.withdrawalMethod === 2">
            <template v-if="!['FamilyShop', 'JustShop'].includes(projectTitle)">
              ...
              <el-form-item :label="$t('姓名')" prop="bankUserName">
                <el-input v-model="formWithdraw.bankUserName" :placeholder="$t('请输入姓名')"></el-input>
              </el-form-item>
              <el-form-item :label="$t('开户行')" prop="bankName">
                <el-input v-model="formWithdraw.bankName" :placeholder="$t('请输入开户行')"></el-input>
              </el-form-item>
              <el-form-item :label="$t('银行卡号')" prop="bankCardNo">
                <el-input v-model="formWithdraw.bankCardNo" :placeholder="$t('请输入银行卡号')"></el-input>
              </el-form-item>
              <el-form-item
                :label="$t('国际代码')"
                prop="swiftCode"
              ...
              >
                <el-input v-model="formWithdraw.swiftCode" :placeholder="$t('请输入国际代码')"></el-input>
              </el-form-item>
              <el-form-item
                :label="$t('路由号码')"
                prop="routingNum"
                v-if="['Argos'].includes(projectTitle)"
              >
                <el-input v-model="formWithdraw.routingNum" :placeholder="$t('请输入路由号码')"></el-input>
              </el-form-item>
              <el-form-item
                :label="$t('账户地址')"
                prop="accountAddress"
                v-if="['Argos'].includes(projectTitle)"
              >
                <el-input v-model="formWithdraw.accountAddress" :placeholder="$t('请输入账户地址')"></el-input>
              </el-form-item>
              <el-form-item
                :label="$t('银行地址')"
                prop="bankAddress"
                v-if="['Argos'].includes(projectTitle)"
              >
                <el-input v-model="formWithdraw.bankAddress" :placeholder="$t('请输入银行地址')"></el-input>
              </el-form-item>
              <el-form-item :label="$t('提现金额')" prop="account">
                <div style="position: relative">
                  <el-input
                  ...
                  <div class="doc-tag">
                    <el-tag type="success" style="margin: 0 4px">{{ formWithdraw.currencyType }}</el-tag>
                  </div>
                </div>
              </el-form-item>
              <el-form-item :label="$t('实际到账')">
          ...
          <template v-if="formWithdraw.withdrawalMethod === 3">
            <template v-if="!['FamilyShop', 'JustShop'].includes(projectTitle)">
              ...
              <el-form-item :label="$t('支付宝账号')" prop="bankCardNo">
                <el-input v-model="formWithdraw.bankCardNo" :placeholder="$t('请输入账号')"></el-input>
              </el-form-item>
              <el-form-item :label="$t('提现金额')" prop="account">
                <div style="position: relative">
                  <el-input
          ...
          <template v-if="formWithdraw.withdrawalMethod === 4">
            <template v-if="!['FamilyShop', 'JustShop'].includes(projectTitle)">
              ...
              <el-form-item :label="$t('微信绑定手机号')" prop="bankCardNo">
                <el-input v-model="formWithdraw.bankCardNo" :placeholder="$t('请输入手机号')"></el-input>
              </el-form-item>
              ...
              <el-form-item label prop>
                <span style="color: #F56C6C">{{ $t('微信') }} {{ $t('提现请联系客服') }}</span>
              </el-form-item>
              <el-form-item label>
                <el-button style="width: 100%" type="primary" @click="submitFrom">{{ $t('提交') }}</el-button>
              </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item>{{ $t('仅能绑定一个收款地址!') }}</el-form-item>
        <el-form-item :label="$t('提现地址')" prop="walletAddress">
...
export default {
  ...
  watch: {
    "formWithdraw.withdrawalMethod"(val) {
      ...
      let withdrawAmountMin, withdrawAmountMax;

      if (val === 3) {
        // Alipay
        withdrawAmountMin =
          this.withdrawLimit.data?.withdrawAmountMinAlipay || 20;
        withdrawAmountMax =
          this.withdrawLimit.data?.withdrawAmountMaxAlipay || 1000;
      } else if (val === 4) {
        // WeChat
        withdrawAmountMin =
          this.withdrawLimit.data?.withdrawAmountMinWeChat || 20;
        withdrawAmountMax =
          this.withdrawLimit.data?.withdrawAmountMaxWeChat || 500;
      } else {
        withdrawAmountMin = this.withdrawLimit.data?.withdrawAmountMin || 10;
        withdrawAmountMax = this.withdrawLimit.data?.withdrawAmountMax || 0;
      }

      this.max = Math.min(this.userBalance, withdrawAmountMax);

      // Update quota reminder message
      ...
      // const withdrawAmountMin = val === 3 || val === 4
      ...
      // this.quotaReminder = this.max <= withdrawAmountMin
      //   ? this.$t('提款范围') + ` ≥ ${withdrawAmountMin} USDT`
      //   : this.$t('提款范围') + ` ${withdrawAmountMin}-${this.max.toFixed(2)} USDT`;
  ...
  methods: {
    ...
    validateFunction(rule, value, callback) {
      ...
      if (this.formWithdraw.withdrawalMethod === 3) {
        // Alipay
        minAmount = this.withdrawLimit.data?.withdrawAmountMinAlipay || 20;
        maxAmount = this.withdrawLimit.data?.withdrawAmountMaxAlipay || 1000;
      } else if (this.formWithdraw.withdrawalMethod === 4) {
        // WeChat
        minAmount = this.withdrawLimit.data?.withdrawAmountMinWeChat || 20;
        maxAmount = this.withdrawLimit.data?.withdrawAmountMaxWeChat || 500;
      } else {
        minAmount = this.withdrawLimit.data?.withdrawAmountMin || 10;
        maxAmount = this.max;
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        getUserBalance().then(balanceRes => {
          ...
          withdrawLimitConfig().then(configRes => {
            ...
            const withdrawAmountMin = configRes.data?.withdrawAmountMin2 || 10;
            const withdrawAmountMax3 = configRes.data?.withdrawAmountMax3 || 0;
            // const withdrawAmountMax2 = configRes.data?.withdrawAmountMax2 || 0
            console.log(this.userBalance);
            this.max =
              this.userBalance < withdrawAmountMin
                ? this.userBalance
                : withdrawAmountMax3;
            if (this.max <= withdrawAmountMin) {
              this.quotaReminder =
              ...
              this.quotaReminder =
                this.$t("提款范围") +
                ` ${withdrawAmountMin}-${this.max.toFloor(2)} USDT`;
            }
