  ...
  <div class="merchandise">
          ...
          <div class="info-m">
            ...
            <el-form-item :label="$t('店铺简介')">
              <el-input
                ...
                @blur="updateMerchantInfo"
              ></el-input>
              <div :style="'padding-left:' + columnWidth" class="introduce">{{ $t("最多200字") }}</div>
            </el-form-item>
          ...
          <div class="info-m">
              ...
              <div style="display: flex">
                <div class="kyc-content">
                  ...
                  <!-- <div class="certification-status">
                    ...
                    <template v-else>
                      <div
                        v-if="!userInfo.phoneverif && info_data.phone !== null"
                      >
                        <el-image
                ...
                <template>
                  <span
                    v-if="!userInfo.phoneverif"
                    class="cursor-pointer"
                    style="margin-left: 10px; color: #0066ff"
                    @click="bindPhoneBefore"
                  >{{ $t("认证手机号") }}</span>
                  <!-- <span
                    v-if="info_data.phone == null"
                    class="cursor-pointer"
                    style="margin-left: 10px; color: #0066ff"
                    @click="changeInfo(1)"
                    >{{ $t("设置") }}</span
            ...
            <el-form-item :label="$t('邮箱')" v-if="!['FamilyMart'].includes(projectTitle)">
              <div style="display: flex">
                ...
                <template>
                  <!-- <span
                    v-if="!userInfo.emailverif && info_data.email != null"
                    class="cursor-pointer"
                    style="margin-left: 10px; color: #0066ff"
                    @click="changeInfo(4)"
                    >{{ $t("认证邮箱") }}</span
    ...
    <el-dialog :title="$t('身份认证')" :visible.sync="userVisible" width="485px">
      <div class="dia-main">
        <el-form>
          ...
          <!-- <el-form-item>
            ...
            <el-input v-model="kycForm.yqm" :placeholder="$t('entryYqm')" clearable maxlength="20" />
          </el-form-item>-->
      ...
      <div
        ...
        @click="renzzheng"
      >{{ kyc_get.status == 0 ? $t("提交申请") : $t("重新申请") }}</div>
    </el-dialog>
    <!--  头像选择  -->
    ...
    <el-dialog
      ...
      @opened="resetInputs"
    >
      <el-form ref="form" :model="formPhone">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="phoneSubmit">
          {{
          $t("提交")
          }}
        </el-button>
...
export default {
  ...
  methods: {
    ...
    onPhoneInput(e) {
      this.formPhone.phone = e.target.value;
    },
    onPassInput(e) {
      this.formPhone.password = e.target.value;
    },
    bindPhoneBefore() {
      this.formPhone.phone = null;
      this.formPhone.password = null;
    ...
    phoneSubmit() {
      ...
      if (this.formPhone.password === "") {
        Toast(this.$t("请输入登录密码"));
        return;
      }

      bindPhone(this.formPhone)
        .then(res => {
          this.getInfo();
          this.info_data.phone = this.formPhone.phone;
