<template>
  <el-dialog
    ...
    width="450px"
    @close="closeDialog"
    class="updateIphoneOrEmail"
  >
    <div v-if="step === 1">
      <div v-if="checkType === 'phone'">
        <p v-if="[1, 2].includes(this.kycType)" style="margin-bottom: 12px">
          {{ $t("为了保障您的账号安全，请验证后进行下一步操作") }}
        </p>
        <p class="mb-8 font-12" style="color: #333; margin-top: 24px">
          {{ $t("当前手机号") }}
        </p>
        <p style="display: flex; width: 100%; justify-content: space-between">
          <span style="font-size: 18px; color: black; padding-right: 24px">{{
        ...
        <div class="flex rounded" style="position: relative; margin-top: 24px">
          <el-input
            ...
            maxlength="6"
            @input="beforeCheckCode($event)"
          />
          <el-button
            style="margin-right: 12px; position: absolute; right: 0; top: 0"
            type="text"
            class="send-button"
        ...
        <el-button
          v-if="[1, 2].includes(this.kycType)"
          class="w-full h-44"
    ...
    <div v-if="step === 0" class="change">
      <div v-if="editTitle === $t('修改手机')">
        <p class="mb-8 font-12" style="color: #333; margin-top: 12px">
          {{ $t("手机号") }}
        </p>
        <div class="c_phone-input-content">
          <img
            :src="require('@/assets/images/login/phone2.png')"
            alt=""
            class="input-icon"
          />
          <el-dropdown
        ...
        <div class="flex rounded" style="position: relative">
          <el-input
            v-model="editPhone.code"
            :placeholder="$t('请输入验证码')"
            maxlength="16"
            @input="checkCode($event, 'editPhone')"
          />
          <el-button
            style="margin-right: 12px; position: absolute; right: 0; top: 0"
            type="text"
        ...
        <PasswordInput
          v-model="editPhone.password"
          @clear="editPhone.password = ''"
      ...
      <div v-else>
        ...
        <div class="flex rounded" style="position: relative">
          <el-input
            ...
            :placeholder="$t('请输入验证码')"
            maxlength="16"
            @input="checkCode($event, 'editEmail')"
          />
          <el-button
            style="margin-right: 12px; position: absolute; right: 0; top: 0"
            type="text"
        ...
        <PasswordInput
          v-model="editEmail.password"
          @clear="editEmail.password = ''"
          :placeholder="$t('请输入登录密码')"
        />
        <el-button
          class="w-full h-44"
    ...
    async getCodePost(type = "phone", step = 0) {
      if (!this.getCode) {
        ...
        if (type === "phone") {
          ...
          target = this.editPhone.phone
            ? this.activeAddress.replace("+", "") + " " + this.editPhone.phone
            : this.userInfo.phone;
        } else {
