    ...
    <div class="search">
      <el-form ref="searchForm" :model="form" inline>
        ...
        <el-form-item :label="$t('下单时间')" prop="time">
          <el-date-picker v-model="form.time" :end-placeholder="$t('结束日期')" :range-separator="$t('至')"
            :start-placeholder="$t('开始日期')" type="datetimerange">
          </el-date-picker>
        </el-form-item>
        <div class="btnList">
          <el-form-item>
          ...
          <template slot-scope="scope">
            <div style="position: relative;padding: 4px;box-sizing: border-box;">
              <el-dropdown size="small" split-button type="primary" @command="handleCommand($event, scope)"
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item command="b"
                    ...
                    size="mini" @click="addGood(scope.row)">
                    {{ $t('采购') }}
                  </el-dropdown-item>
              ...
              <div v-if="(scope.row.purchStatus == 0) && scope.row.payStatus == 1 && scope.row.status === 1"
                style="position: absolute;right: 5px;top: 3px;width: 6px;height: 6px;border-radius: 50%;background-color: red;z-index: 999">
              </div>
            </div>
          </template>
        </el-table-column>
          ...
          <el-table :data="detailData" max-height="250px">
            <el-table-column :label="$t('封面图')">
              <template slot-scope="{row}">
                <el-image :alt="row.goodsName" :preview-src-list="[row.goodsIcon]" :src="row.goodsIcon" fit="contain"
      ...
      <div class="detail">
        ...
        <div class="main-price">
          ...
          <div class="item">
            ...
            <FormatNumberShow :data="orderInfo.sellerFees" :currency="true" />
          </div>
          <div class="item">
          ...
          <div class="main-content">
            <div class="left">
              ...
              <div class="item">
                <div>{{ $t('采购状态') }}</div>
                <div>
                  <span v-if="parseInt(orderInfo.purchStatus) === 0" :style="{
                    color: '#fff',
                    padding: '2px 5px',
            ...
            <div class="left">
              <div class="item">
                ...
                <div style="position: relative">
                  <div>{{ orderInfo.goodType == "virtual" ? "--" : orderInfo.contacts }}</div>
              ...
              <div class="item">
                <div>{{ $t('地址') }}</div>
                <div>{{ orderInfo.goodType == "virtual" ? "--" : orderInfo.address }}</div>
              </div>
              <div class="item">
                <div>{{ $t('国家') }}</div>
                <div>{{ orderInfo.goodType == "virtual" ? "--" : getCountry(orderInfo.country) }}</div>
              </div>
              <div class="item">
                <div>{{ $t('城市') }}</div>
                <div>{{ orderInfo.goodType == "virtual" ? "--" : getCity(orderInfo.city) }}</div>
              </div>
            </div>
            <div class="right">
              <div class="item">
              ...
              <div class="item">
                <div>{{ $t('省区') }}</div>
                <div>{{ orderInfo.goodType == "virtual" ? "--" : getState(orderInfo.province) }}</div>
              </div>
              <div class="item">
                <div>{{ $t('邮编') }}</div>
                <div>{{ orderInfo.goodType == "virtual" ? "--" : orderInfo.postcode }}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="main">
            <el-table :data="detailData" max-height="250px">
              <el-table-column :label="$t('查看图片')">
                <template slot-scope="scope">
                  <el-image style="width: 36px; height: 36px" :src="scope.row.goodsIcon"
          ...
          <div class="order-price">
            ...
            <div class="main">
              ...
              <div class="item top">
                ...
                <FormatNumberShow :data="orderInfo.prizeReal + orderInfo.tax + orderInfo.sellerFees" :currency="true" />
  ...
  methods: {
    ...
    addGood(row) {
      ...
      api_order_info_action_post({ orderId: row.id }).then((e) => {
        console.log(e.data.orderInfo, "采购");
    ...
    openOrder(row) {
      this.userInfo2 = row;
      api_order_info_action_post({ orderId: row.id }).then((e) => {
        console.log(e.data.orderInfo.sellerFees, "详情数据")
        console.log(e.data.orderInfo, "运费")
