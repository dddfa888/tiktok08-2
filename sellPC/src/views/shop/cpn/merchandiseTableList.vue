<template>
  <div class="table-list">
    ...
    <el-card>
      <div class="btn">
        <el-button type="primary" @click="showBatch($t('批量上架'),1)">{{ $t('批量上架') }}</el-button>
        <el-button
          type="danger"
          @click="showBatch($t('批量下架'),0)"
          v-if="!['JustShop','FamilyShop'].includes(setting.projectTitle)"
        >{{ $t('批量下架') }}</el-button>
        <el-button type="primary" @click="batchEdit($t('批量修改'))">{{ $t('批量修改') }}</el-button>
        <el-button
          type="danger"
      ...
      >
        ...
        <el-table-column :label="$t('二级分类')" align="center" prop="secondaryCateName"></el-table-column>
        ...
        <el-table-column :label="$t('标签')" :min-width="columnWidth" align="center" prop="time">
          <template slot-scope="scope">
            <!--            {{row.isShelf}}-->
            <!--            {{scope.row.isShelf}}-->
          ...
          <template slot-scope="scope">
            <div style="display: flex;justify-content: center;">
              ...
              <el-button
                type="danger"
                v-if="![].includes(setting.projectTitle)"
                @click="delVisible = true,shachu=[],shachu=[scope.row.id]"
              >{{ $t('删除') }}</el-button>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="$t('删除提示')" :visible.sync="delVisible" width="450px">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="api_order_listGoods_delete(shachu,true)">{{ $t('确定') }}</el-button>
